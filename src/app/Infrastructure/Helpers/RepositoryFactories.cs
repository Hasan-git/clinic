...
namespace Clinic.Infrastructure.Helpers
{
    ...
    public class RepositoryFactories
    {
        ...
        {
            // var appCache = IoC.Resolve<IAppMicroCache>();
            return new Dictionary<Type, Func<DbContext, object>>{
                     {typeof(IAppointmentRepository), dbcontext => new AppointmentRepository(dbcontext)},
                     {typeof(IAssistantRepository), dbcontext => new AssistantRepository(dbcontext)},
                     {typeof(IClinicRepository), dbcontext => new ClinicRepository(dbcontext)},
                     {typeof(IConsultationRepository), dbcontext => new ConsultationRepository(dbcontext)},
                     {typeof(IDoctorRepository), dbcontext => new DoctorRepository(dbcontext)},
        ...
        /// </remarks>
        public Func<DbContext, object> GetRepositoryFactoryForEntityType<T>() where T : class, IBaseModel
        {
        ...
        protected virtual Func<DbContext, object> DefaultEntityRepositoryFactory<T>() where T : class, IBaseModel
        {
