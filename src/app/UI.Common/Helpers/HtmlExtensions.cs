...
namespace Clinic.UI.Common.Helpers
{
    public static class HtmlExtensions
    {
        ...
        public static HtmlString MetaTag(this HtmlHelper html, string tagName, string name, string value, int priority = 1)
        {
            ...
            name = name.ToLower();
            if (requiredTags.All(i => i.Name != name))
                requiredTags.Add(new MetaTagInclude() { TagName = tagName.ToLower(), Name = name, Value = value, Priority = priority });
            return null;
        }
        ...
        public static HtmlString Style(this HtmlHelper html, string path, int priority = 1)
        {
            ...
            if (requiredStyles.All(i => i.Path != path))
                requiredStyles.Add(new ResourceInclude { Path = path, Priority = priority });
            return null;
        }
